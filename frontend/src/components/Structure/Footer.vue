					...
					<div class="container-fluid">
						<div class="row" style="font-size: 0.7rem">
							<div v-for="(column, index) in columnedStaticPages" :key="index" class="col-md-6">
								<router-link class="text-dark d-block" :to="{ name: 'StaticPage', params: { id: page.id }, query : { title: $root.toGreeklish(page.Name).replace(/ /g, '-') } }" :key="page.id" v-for="page in column">{{ page['Name'] }}</router-link>
							</div>
						</div>
					</div>
				</div>
