<template>
	<div class="col-12 col-lg-6 mb-3" :class="{'article-card-bordered': isLeft }">
		...
		<div class="row py-4">
			...
			<button @click="$parent.editArticleId = article.id" type="button" class="btn px-4 bg-info text-white text-uppercase font-weight-bold mx-auto" style="border-radius: 2rem;" data-toggle="modal" data-target="#articleEditModal">
				{{ $t('Edit') }}
			</button>
		</div>
	</div>
