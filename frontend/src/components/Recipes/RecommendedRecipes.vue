							...
							<div class="container">
								<div class="row">
									<template v-if="noAdCards">
										<RecipeCard v-for="(recipe, index2) in recipesPart" :key="'tripleRecipeCard' + recipe.id" :recipe="recipe" :class="{ 'v-border-left': index2 > 0}"/>
									</template>
									<template v-else>
										<template v-for="(recipe, index2) in recipesPart">
											<template v-if="recipe.hasOwnProperty('article')">
												<RecipeCard :recipe="recipe['article']" :key="'triple-recipe' + recipe['article'].id" :class="{ 'v-border-left': index2 > 0}"/>
											</template>
											<template v-else>
												<AdCard :ad="recipe['ad']" listType="recipes" :key="'triple-ad' + recipe['ad'].id + '-' + index" :class="{ 'v-border-left': index2 > 0}"/>
											</template>
										</template>
									</template>
								</div>
		...
		methods: {
			getArticles: function () {
				...
				if (this.ingredientsIncluded.length)
					requestUrl += '&ingredients=' + this.ingredientsIncluded.join();
				if (this.location)
					requestUrl += '&' + this.location;
